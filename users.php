...
<?php
	...
	while ($row_user = $result->fetch_assoc()) { 
		...
		<tr>
			...
			<td><select id="<?php echo trim($row_user['login']) ?>^role" multiple onchange="javascript: console.log(this.id, implode(',', $(this).val())); ">
			<?php
				foreach ($roles as $key => $value) {
					if (in_array($key, $user_roles)) $selected = "selected='selected'";
					else $selected = "";
