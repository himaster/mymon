...
$result = $dbconnection->query("SELECT id FROM $db.`users` WHERE login ='{$login}' LIMIT 1;") or die($dbconnection->error);
$uid = $result->fetch_assoc()['id'];
$result = $dbconnection->query("SELECT `id`, `name` FROM $db.`roles`") or die($dbconnection->error);
while ($row = $result->fetch_assoc()) {
    if ($_GET[$row['name']] == "on") {
        $rid = $row['id'];
        $dbconnection->query("INSERT INTO $db.`user_roles`(`user_id`, `role_id`)
                              VALUES ('$uid', '$rid');") or die($dbconnection->error);
    }
}
$result = $dbconnection->query("UPDATE $db.`users`
                                SET approvied = '1'
