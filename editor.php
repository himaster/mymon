...
while ($row_user = $result->fetch_assoc()) {
	echo "<tr>";
	echo "<td><input id=\"name_".trim($row_user['servername'])."\" type=\"text\" value=\"".trim($row_user['servername'])."\" onchange=\"javascript: editor(this.value, this.id); \"></td>";
	echo "<td><input id=\"ip_".trim($row_user['servername'])."\" type=\"text\" value=\"".trim($row_user['ip'])."\" onchange=\"javascript: editor(this.value, this.id); \"></td>";
	echo "<td><input id=\"db_".trim($row_user['servername'])."\" type=\"checkbox\" ";
	if (trim($row_user['db']) == 1) echo "checked "; 
	echo "onchange=\"javascript: editor(this.checked, this.id); \"></td>";
