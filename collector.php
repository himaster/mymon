...
function child_()
{
    ...
    $result = $$mysql_conname->query("UPDATE `mymon`.`stats` SET
    	                              `la`='".la($$ssh_conname, $serverip)."' ,
    ...
    if ($errs == 1) {
        $result = $$mysql_conname->query("UPDATE `mymon`.`stats` SET
        	                              `500`='".err500($$ssh_conname, $serverip)."' ,
        	                              `timestamp`=CURRENT_TIMESTAMP WHERE `ip`='".$serverip."';");
    } else {
    ...
    if ($elastic == 1) {
        $result = $$mysql_conname->query("UPDATE `mymon`.`stats` SET
        	                              `elastic`='".elastic($$ssh_conname, $serverip)."' ,
        	                              `timestamp`=CURRENT_TIMESTAMP WHERE `ip`='".$serverip."';");
    } else {
    ...
    if ($mysql == 1) {
        $result = $$mysql_conname->query("UPDATE `mymon`.`stats` SET
        	                              `locks`='".locks($$ssh_conname, $serverip)."' ,
        	                              `timestamp`=CURRENT_TIMESTAMP WHERE `ip`='".$serverip."';");
    } else {
    ...
    if ($mon == 1) {
        $result = $$mysql_conname->query("UPDATE `mymon`.`stats` SET
        	                              `mongo`='".mongo($$ssh_conname, $serverip)."' ,
        	                              `timestamp`=CURRENT_TIMESTAMP WHERE `ip`='".$serverip."';");
    } else {
    ...
    if ($red == 1) {
        $result = $$mysql_conname->query("UPDATE `mymon`.`stats` SET
        	                              `redis`='".redis($$ssh_conname, $serverip)."' ,
        	                              `timestamp`=CURRENT_TIMESTAMP WHERE `ip`='".$serverip."';");
    } else {
...
function elastic($connection, $serverip)
{
    $str = ssh2_return($connection, "date1=\$((\$(date +'%s%N') / 1000000));
                                     host=\$(ip -f inet addr show eth1 | grep -Po 'inet \K[\d.]+')
									 curl -sS -o /dev/null -XGET http://\$host:9200/_cluster/health?pretty;
...
{
    $str = ssh2_return($connection, "date1=\$((\$(date +'%s%N') / 1000000));
                                     query='printjson(db.serverStatus().connections.current)';
									 mongo admin --quiet --eval '\$query' 1>/dev/null;
									 date2=\$((\$(date +'%s%N') / 1000000));
									 echo -n \$((\$date2-\$date1));");
    if ($str == 'Timeout') {
...
function sighandler($signo)
{
    ...
    switch ($signo) {
        ...
        default:
            common_log('Signal $signo received. Skipping.');
            break;
    }
}
