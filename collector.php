...
function child_() {
	...
	#$result = $$mysql_conname->query("UPDATE `mymon`.`stats` SET `la`='' WHERE `ip`='" .$serverip. "';");
	$result = $$mysql_conname->query("UPDATE `mymon`.`stats` SET `la`='' WHERE `ip`='" .$serverip. "'; UPDATE `mymon`.`stats` SET `la`='" .la($$ssh_conname, $serverip). "' WHERE `ip`='" .$serverip. "';");
